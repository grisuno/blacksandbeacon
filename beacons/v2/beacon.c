...
/* Mesh functions would be implemented here, but for this refactor
 * we keep the same structure as v1 with mesh stubs. A production
 * mesh implementation would add UDP broadcast listeners, peer
 * relay logic, and message deduplication. */
...
int main(void) {
    ...
    if (bsb_config_load_default(&cfg, cfg_err, sizeof(cfg_err)) != 0) {
        ...
        fprintf(stderr, "hint: run `make config` and `make`, then invoke ./build/beacon-v2\n");
        return 1;
    }
    ...
    bsb_output_cleanup();
    pthread_mutex_destroy(&g_mesh.peers_mutex);
    pthread_mutex_destroy(&g_mesh.seen_mutex);
    return 0;
}
